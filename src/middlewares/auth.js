...
export function checkAdmin(req, res, next) {
    if (req.user != undefined && req.user.role === 'admin' || req.user.role === 'premium' )  {
        return next();
    } else {
...
export function checkAuthToDelete(req, res, next) {
    ...
    CustomError.createError({
        name: "Authentication Error",
        cause: "the user does not have permission to access this site.",
        message: "only admin or premium users can delete a product",
        code: EErrors.AUTH_ERROR,
        },
        logger.error("AUTH_ERROR")
        );
